...
class ReserverFactory extends Factory
{
    ...
    {
        return [
            // dateDebut	heureDebut	duree	etatReservation	ressource_id	user_id	created_at	updated_at	
            'dateDebut' => fake()-> date($format = 'Y-m-d', $max = 'now'),
            'heureDebut' => fake() -> time($format = 'H:i:s', $max = 'now'),
            'duree' => fake() ->numberBetween($min = 1, $max = 20),
            'etatReservation' => fake()->boolean(),
